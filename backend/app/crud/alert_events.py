...
from app.models import AlertEvent, AlertEventPublic, AlertRule, AlertStatistics, AlertStatisticsRuleItem, AlertStatisticsSeverityItem, NotificationChannel
...
def get_alert_statistics(*, session: Session) -> AlertStatistics:
    ...
    sent = session.exec(select(func.count()).select_from(AlertEvent).where(AlertEvent.status == "sent")).one() or 0
    failed = total - sent

    last_24h = session.exec(
        select(func.count()).select_from(AlertEvent).where(AlertEvent.triggered_at >= now - timedelta(hours=24))
    ).one() or 0
    last_7d = session.exec(
        select(func.count()).select_from(AlertEvent).where(AlertEvent.triggered_at >= now - timedelta(days=7))
    ).one() or 0
    ...
    by_severity = [AlertStatisticsSeverityItem(severity=sev, count=cnt) for sev, cnt in sev_rows]
    ...
    by_rule = [AlertStatisticsRuleItem(rule_name=name, count=cnt) for name, cnt in rule_rows]
