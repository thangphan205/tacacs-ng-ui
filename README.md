...
A [Model Context Protocol](https://modelcontextprotocol.io) endpoint lets an LLM client (Claude Desktop, Claude Code, Antigravity, Gemini, Cursor, Windsurf) inspect TACACS+ entities, render config previews and diffs, and syntax-check config text with the real `tac_plus-ng -P` parser. On by default (`MCP_ENABLED=true`; set to `false` to disable); authenticated with dedicated, revocable API keys, with secrets redacted from all output.

Keys are issued at one of two access levels:

| Level | What the client can do |
|---|---|
| **Read-only** | Inspect entities and settings, render previews, diffs and sections, syntax-check config. Changes nothing. |
| **Read-write** | The above, plus creating, updating and deleting TACACS+ entities. Requires the key to belong to a superuser. |

**Neither level can deploy.** There is no tool that saves a config file, activates one, or reloads tac_plus-ng. An entity edited over MCP sits in the database until a human opens **TACACS Configs** and presses *Generate*, then *Activate* — and that page now flags when the database has drifted from the running config, so a pending change is visible rather than silent.
...
8. **LLM-Assisted Administration**: ✅ Shipped in v0.6.0 — an MCP server exposing fifteen tools, a tac_plus-ng syntax reference resource, an entity schema resource, and an authoring prompt, so an LLM client can inspect entities, preview and diff configs, syntax-check candidate config text against the real parser, and — with a read-write key — create, update and delete entities. Backed by scoped, revocable API keys with secret redaction on every response. Deploying a config (saving, activating, reloading the daemon) is deliberately and permanently out of scope, enforced by a guard test rather than convention.
